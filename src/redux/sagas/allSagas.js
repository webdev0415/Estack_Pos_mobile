...
function* createPos({ payload }) {
  ...
  try {
    const { data } = yield call(api.post, endpoints.CREATE_POS_ENDPOINT, payload);
    yield AsyncStorage.multiSet([['accessToken', data.accessToken],['refreshToken', data.refreshToken]]),
...
function* setPosDetails({ payload }) {
  ...
  try {
    const { data } = yield call(api.put, endpoints.POS_DETAILS_ENDPOINT, payload, Authorization(accessToken));
    yield all([
...
function* getFullDetails() {
  const accessToken = yield AsyncStorage.getItem('accessToken');
  yield put(actions.setPosWalletAction({
    points: 0,
    coupons: 0,
    gmv: 0,
  }));
  ...
  try {
    const { data } = yield call(api.get, endpoints.POS_DETAILS_ENDPOINT, Authorization(accessToken));
    yield all([
...
function* denyCoupon({ payload }) {
  ...
  try {
    const { data } = yield call(api.put, `${endpoints.DENY_COUPON_ENDPOINT}${uuid}`, {}, Authorization(accessToken));
...
function* approveCoupon({ payload }) {
  ...
  try {
    yield call(api.put, `${endpoints.APPROVE_COUPON_ENDPOINT}${uuid}`, {}, Authorization(accessToken));
...
function* getWallet() {
  ...
  try {
    const { data } = yield call(api.get, endpoints.POS_WALLET_ENDPOINT, Authorization(accessToken));
...
function* forgotPassword({ payload }) {
  try {
    const { data } = yield call(api.post, endpoints.FORGOT_PASSWORD_ENDPOINT, payload)
    if (data === true) {
...
function* getCustomer({ payload }) {
  ...
  try {
    ...
    const { data } = yield call(api.get, `${endpoints.GET_CUSTOMER_INFOR_ENDPOINT}${_id}`, Authorization(accessToken));
